---
title: Larval respirometry (photosynthesis & respiration) with LoLinR for bleaching pairs 2021
author: "AS Huffmyer"
date: '2021'
output:
  html_document:
...
```{r, warning=FALSE, message=FALSE}
path.p<-"Bleaching_Pairs2021/Data/Respiration/Runs" #location of files
```
...
```{r, warning=FALSE, message=FALSE}
Sample.Info<-read.csv("Bleaching_Pairs2021/Data/Respiration/Resp_Sample_Info.csv") #information on life stage, number of individuals, runs, etc.
rename <- Sample.Info$Chamber.ID
samp <- Sample.Info$Sample.ID
run <- str_sub(file.names, 10, 13) #grab run from file name
date <- str_sub(file.names, 1, str_length(file.names)-30) #grab date from file name

#load in respiration start times as a list the same length as the number of files
starttimes<-read.csv("Bleaching_Pairs2021/Data/Respiration/starttimes.csv") #read in file of starting R and P phase times
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Resp.Data.N))){
    ...
    pdf(paste0("Bleaching_Pairs2021/Output/Respiration/RespirationPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output file names
    plot(model)
    dev.off()
...
Removed column B6 due to missing sensor.  
...
write.csv(Resp.R, paste0("Bleaching_Pairs2021/Output/Respiration/Respiration_rates_Pairs2021.csv", sep="")) #save respiration rate data
```

```{r, warning=FALSE, message=FALSE}
Resp.Rates <- read.csv(file = "Bleaching_Pairs2021/Output/Respiration/Respiration_rates_Pairs2021.csv") #read file back in so slopes don't have to be generated every time
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Photo.Data.N))){
    ...
    pdf(paste0("Bleaching_Pairs2021/Output/Respiration/PhotosynthesisPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output files
    plot(model)
    dev.off()
...
write.csv(Photo.R, paste0("Bleaching_Pairs2021/Output/Respiration/Photosynthesis_rates_Pairs2021.csv", sep="")) #save P rates
```

```{r, warning=FALSE, message=FALSE}
Photo.Rates <- read.csv(file = "Bleaching_Pairs2021/Output/Respiration/Photosynthesis_rates_Pairs2021.csv") #import P rates
...
Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("Bleaching_Pairs2021/Output/Respiration/oxygen_P_R_calc.csv", sep="")) #save final file
```
