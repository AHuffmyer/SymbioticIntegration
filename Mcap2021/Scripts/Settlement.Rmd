...
settle_plot<-settle %>%
    ...
    geom_text(x=3, y=14, label="Date p<0.001", color="black", size=8)+
    ylim(0, 20)+
    theme_classic() + 
    theme(
...
settle_plot2<-settle %>%
    ...
    geom_text(x=as.POSIXct("2021-06-24"), y=28, label="Date p<0.001", color="black", size=5)+
    ylim(0, 30)+
    theme_classic() + 
    theme(
      legend.position="none",
...
settlemodel<-lmer(Settled~Treatment*Date + (1|Tank), data=settle) #run as random
