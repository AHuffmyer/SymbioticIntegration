...
chl.total_protein_plot_b<-df_mean %>%
    ...
    geom_text(x=1.5, y=35, label="p=0.455", color="darkgray", size=8)+
    theme_classic() + 
    theme(
      legend.position="none",
      axis.title=element_text(face="bold", size=14),
...
chl.total_cell_plot_b<-df_mean %>%
    ...
    geom_text(x=1.5, y=0.06, label="p=0.935", color="darkgray", size=8)+
    theme_classic() + 
    theme(
      legend.position="none",
      axis.title=element_text(face="bold", size=14),
...
```{r}
chl.plot<-plot_grid(chl.total_protein_plot_b, chl.total_cell_plot_b, nrow=1, ncol=2, label_y = 1, labels=c("A", "B"), label_size=18)
ggsave(chl.plot, file="Mcap2021/Figures/Chlorophyll_panel.png", width=8, height=5)
```