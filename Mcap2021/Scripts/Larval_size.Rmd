...
size_plot_3b<-size %>%
    ...
    geom_point(aes(fill=treatment), pch = 21, size=8, position = position_dodge(0.2)) + 
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se, group=treatment), pch = 21, size=2, position = position_dodge(0.2), width=0) + 
...
#Save plot
ggsave(size_plot_3b, file="Mcap2021/Figures/size.png", w=4, h=4)
```
...
test<-lmer(volume.mm3~treatment+(1|tube.id:treatment), data=model_data)
summary(model2)
anova(test)
