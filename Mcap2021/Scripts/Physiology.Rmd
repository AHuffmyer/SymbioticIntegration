...
Protein.Plot <- df_protein%>%
  ...
  xlab("Temperature") + ylab(expression(bold("Soluble Protein " (ug)))) + #Axis titles
...
First, calculate protein per uL from the "Holobiont" tube for each sample, which is the tube the sample was taken from. 

```{r}
protein<-read.csv("Mcap2021/Output/Protein/protein_output.csv")

protein<-dplyr::rename(protein, TubeID=Tube.ID)
...
Third, normalize cells per uL / host protein per uL to obtain cells per ug host protein
...
df_cells%>%
  ...
  kbl(caption="Descriptive statistics of Symbiodiniaceae cell densities per µg host protein")%>%
  kable_classic(full_width=FALSE, html_font="Arial")%>%
  row_spec(0, bold = TRUE) 
```

Within lower part of range seen in Goodbody-Gringley et al. 2024 (10.1038/s41598-024-54217-3) at ~1000-15000 cells per ug protein. Also matches range seen in early infection stages in anemones in Xiang et al.2020 (10.1038/s41467-019-13963-z). 
...
sym_plot <- df_cells%>%
  ...
  geom_text(x=1.5, y=0, label="p=0.345", color="darkgray", size=4)+
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Symbiont Density (cells µg host protein"^-1, ")"))))+
  custom_theme; sym_plot 

# output data for supplement 
df_cells%>%
  filter(!Sample.ID=="H")%>%
  select(TubeID.x, Sample.ID, Treatment, cells.ugprotein)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS3a_data.csv")
```
...
Bring in volume data from symbiont tubes, where the samples came from.  
```{r}
...
First, calculate protein per uL from the host fraction. 
...
Next, divide chlA and chlC2 (ug.uL) by host protein (ug.uL) for each sample.  
...
chl_prot_plot <- df_chl%>%
  ...
  scale_colour_manual(values=c("blue", "red"))+
  ylim(0,500)+
  geom_text(x=1.5, y=0, label="p=0.225", color="darkgray", size=4)+
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Total Chlorophyll (µg µg host protein"^-1, ")"))))+
  custom_theme; chl_prot_plot 

# output data for supplement 
df_chl%>%
  select(Sample.ID, Tube.ID, Treatment, total.chl.ug.protein.ug)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS3b_data.csv")
```
...
chl_cells_plot <- df_chl%>%
  ...
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Total Chlorophyll (µg cell"^-1, ")"))))+
  custom_theme; chl_cells_plot 

# output data for supplement 
df_chl%>%
  select(Sample.ID, Tube.ID, Treatment, total.chl.ug.cell)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS3c_data.csv")
```
...
Remove outlier sample H. 
```{r}
df_carbs<-df_carbs%>%
  filter(!Sample.ID=="H")
```

Calculate carbs in the symbiont fraction by calculating the difference between host and holobiont values.  
```{r}
df_carbs<-df_carbs%>%
  select(Sample.ID, Treatment, Fraction, Carb.prot)%>%
  ungroup()%>%
  select(!Tube.ID)%>%
  pivot_wider(values_from=Carb.prot, names_from=Fraction)%>%
  mutate(Symbiont=Holobiont-Host)%>%
  pivot_longer(values_to="Carb.prot", names_to="Fraction", cols=c(Holobiont:Symbiont))
```
...
Test for differences with a t test for symbiont carbohydrates.  
...
leveneTest(Carb.prot~Treatment, data=model_data)
```
There is no significant difference in symbiont carbohydrates. 
...
carb.host.prot <- df_carbs%>%
  ...
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Host Carbohydrates (µg µg protein"^-1, ")"))))+ #Axis titles
...
df_carbs%>%
  ...
  dplyr::summarise(mean=mean(Carb.prot, na.rm=TRUE))%>%
  pivot_wider(values_from=mean, names_from=Treatment)%>%
  mutate(perc_diff=((High-Ambient)/Ambient)*100)
...
df_carbs%>%
  ...
  filter(Carb.prot>0)%>%
  select(Sample.ID, Treatment, Fraction, Carb.prot)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS3d_data.csv")
```
...
Plot symbiont carbohydrates across temperatures.   

```{r}
carb.sym.prot <- df_carbs%>%
  filter(Fraction=="Symbiont")%>%
  filter(Carb.prot>0)%>%
  ...
  #ylim(0,17)+
   scale_fill_manual(values=c("blue", "red"))+
  scale_colour_manual(values=c("blue", "red"))+
  geom_text(x=1.5, y=0, label="p=0.050", color="darkgray", size=4)+
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Symbiont Carbohydrates (µg µg protein"^-1, ")"))))+ #Axis titles
  custom_theme; carb.sym.prot 
```

Variable and losing many values <0. 
...
Test for differences in symbiont carbs per cell with a t test.   
...
Plot carbs by fractions normalized to cell density.   
...
carbs.cell.host <- df_carbs%>%
  ...
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Host Carbohydrates (µg cell"^-1, ")")))) + #Axis titles
...
df_carbs%>%
  filter(Fraction=="Host")%>%
  filter(carbs.cell>0)%>%
  select(Sample.ID, Treatment, Fraction, carbs.cell)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS3e_data.csv")
```
...
carbs.cell.sym <- df_carbs%>%
  ...
  scale_colour_manual(values=c("blue", "red"))+
  ylim(0,0.02)+
  geom_text(x=1.5, y=0, label="p=0.001", color="black", size=4)+
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Symbiont Carbohydrates (µg cell"^-1, ")")))) + #Axis titles
  custom_theme; carbs.cell.sym 
```

This is interesting, but the data is highly variable with over half as negative values. This is not reliable. A larger symbiont pellet is needed to reliably measure carbohydrate content.  
...
subset_carbs_host<-df_carbs%>%
  ...
  filter(Fraction=="Host")%>%
  dplyr::rename(Host.carb.ug.prot.ug=Carb.prot, Host.carbs.ug.cell=carbs.cell)%>%
  select(!Fraction)
...
```{r}
p<-master%>%
  ggpairs(., columns=3:9, ggplot2::aes(colour=Treatment)) #try to bold significant stats
...
Host carbs per cell ~ cells per host protein 
```{r}
cor2<-master%>%
  ...
  geom_text(x=1500, y=0.007, label="r=0.636, p=0.001", colour="black")+
  theme_classic()+
  theme(text=element_text(size=14, face="bold"))+
...
all_plots<-plot_grid(sym_plot2, chl_prot_plot2, chl_cells_plot2, carb.host.prot2, carbs.cell.host2, nrow=1, ncol=5, label_y=1, labels=c("A", "B", "C", "D", "E"), label_size=18)

ggsave(all_plots, file="Mcap2021/Figures/physiology_panel.png", width=14, height=5)
```
