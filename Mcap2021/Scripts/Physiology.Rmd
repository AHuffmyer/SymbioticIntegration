...
Run ANOVA on cells per protein by treatment.    
```{r}
...
sym_plot <- df_cells%>%
  ...
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Cells ug protein"^-1, ")"))))+
  custom_theme; sym_plot 

#ggsave("Mcap2021/Figures/symbiont_density.png", sym_plot, width=4, height=5)
```
...
leveneTest(residuals(chla_prot_model)~Treatment, data=df_chl)
```
...
car::leveneTest(residuals(chlc2_prot_model)~Treatment, data=df_chl)
```
...
car::leveneTest(residuals(chl.total_prot_model)~Treatment, data=df_chl)
```
...
chl_prot_plot <- df_chl%>%
  ...
  scale_colour_manual(values=c("blue", "red"))+
  ylim(0,210)+
  geom_text(x=1.5, y=0, label="p=0.371", color="darkgray", size=4)+
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Total Chlorophyll (ug ug protein"^-1, ")"))))+
  custom_theme; chl_prot_plot 

#ggsave("Mcap2021/Figures/chl_protein.png", chl_prot_plot, width=4, height=5)
```
...
car::leveneTest(residuals(chla_cell_model)~Treatment, data=df_chl)
```
...
car::leveneTest(residuals(chlc2_cell_model)~Treatment, data=df_chl)
```
...
car::leveneTest(residuals(chl.total_cell_model)~Treatment, data=df_chl)
```
...
chl_cells_plot <- df_chl%>%
  ...
  scale_colour_manual(values=c("blue", "red"))+
  ylim(0,0.55)+
  geom_text(x=1.5, y=0, label="p=0.712", color="darkgray", size=4)+
  xlab(expression(bold("Temperature"))) + ylab(expression(bold(paste("Total Chlorophyll (ug cell"^-1, ")"))))+
  custom_theme; chl_cells_plot 

#ggsave("Mcap2021/Figures/chl_cells.png", chl_cells_plot, width=4, height=5)
```
...
wilcox.test(ratio~Treatment, data=carb_ratio)
kruskal.test(ratio~Treatment, data=carb_ratio)
t.test(ratio~Treatment, data=carb_ratio)
```

There is a slight but non-significant difference in Host:Holobiont ratios of carbohydrates at high temperature. There are more carbohydrates in the host fraction out of the total carbohydrates at high temperature. 
...
carb_ratio_plot <- carb_ratio%>%
  ...
  ylim(0,0.7)+
   scale_fill_manual(values=c("blue", "red"))+
  scale_colour_manual(values=c("blue", "red"))+
  geom_text(x=1.5, y=0, label="p=0.108", color="darkgray", size=4)+
...
wilcox.test(Carb.prot~Treatment, data=model_data)
kruskal.test(Carb.prot~Treatment, data=model_data)
t.test(Carb.prot~Treatment, data=model_data)
```
There is no significant difference in host carbohydrates, but it is close! 
...
carb.host.prot <- df_carbs%>%
  ...
  ylim(0,8)+
   scale_fill_manual(values=c("blue", "red"))+
  scale_colour_manual(values=c("blue", "red"))+
  geom_text(x=1.5, y=0, label="p=0.099", color="darkgray", size=4)+
...
carb.holobiont.prot <- df_carbs%>%
  ...
  ylim(0,17)+
   scale_fill_manual(values=c("blue", "red"))+
  scale_colour_manual(values=c("blue", "red"))+
  geom_text(x=1.5, y=0, label="p=0.217", color="darkgray", size=4)+
...
Test for differences in host carbs per cell with an anova.  
...
kruskal.test(carbs.cell~Treatment, data=cells_data_model)
t.test(carbs.cell~Treatment, data=cells_data_model)
```

There is no difference in host carbs per cell by treatment. 

Test for differences in holobiont carbs per cell with an anova.  
...
There is a difference in carbs in the holobiont when normalized to cell density. 
...
carbs.cell.holo <- df_carbs%>%
  ...
  scale_colour_manual(values=c("blue", "red"))+
  ylim(0,0.04)+
  geom_text(x=1.5, y=0, label="p=0.046", color="black", size=4)+
