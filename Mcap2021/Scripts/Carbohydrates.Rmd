...
Ratio.Plot_b<-ratios %>%
    ...
    dplyr::summarise(mean=mean(ratio), se=sd(ratio)/sqrt(length(ratio)))%>%
    ...
    xlab("Treatment") + 
    ylim(0.05, 0.3)+
    ylab(expression(bold(paste("Host:Holobiont Carbohydrates"))))+
     geom_text(x=1.5, y=0.08, label="p=0.046", color="black", size=8)+
    theme_classic() + 
    theme(
...
Plot total host carbohydrates with points and error bars. 
```{r}
carb.host.prot<-Samples_norm %>%
    filter(Fraction=="Host")%>%
    dplyr::group_by(Treatment)%>%
    dplyr::summarise(mean=mean(Carb.normalized), se=sd(Carb.normalized)/sqrt(length(Carb.normalized)))%>%
    ...
    xlab("Treatment") + 
    ylim(3, 7)+
    ylab(expression(bold(paste("Host Carbohydrates (ug ug protein"^-1, ")"))))+
    geom_text(x=1.5, y=3.5, label="p=0.078", color="darkgray", size=8)+
    theme_classic() + 
    theme(
      legend.position="none",
      axis.title=element_text(face="bold", size=14),
...
Plot host carbohydrates normalized to cell counts 

Plot symbiont density with plot and error bars.   
```{r}
Carb.Plot.cells.host_b<-cells_data_host %>%
    filter(!Sample.ID=="H")%>%
    dplyr::group_by(Treatment)%>%
    dplyr::summarise(mean=mean(Carb.ug.cell), se=sd(Carb.ug.cell)/sqrt(length(Carb.ug.cell)))%>%
    ...
    xlab("Treatment") + 
    ylim(0.002, 0.01)+
    geom_text(x=1.5, y=0.003, label="p=0.769", color="darkgray", size=8)+
    ylab(expression(bold(paste("Host Carbohdyrates (ug symbiont cell"^-1, ")"))))+
    theme_classic() + 
    theme(
      legend.position="none",
      axis.title=element_text(face="bold", size=14),
...
```{r}
carbs.plot<-plot_grid(carb.host.prot, Carb.Plot.cells.host_b, Ratio.Plot_b, nrow=1, ncol=3, label_y=1, labels=c("A", "B", "C"), label_size=18)
ggsave(carbs.plot, file="Mcap2021/Figures/Carbs_panel.png", width=12, height=5)
```
