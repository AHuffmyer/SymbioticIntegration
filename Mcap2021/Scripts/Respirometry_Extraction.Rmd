...
```{r, warning=FALSE, message=FALSE}
path.p<-"Mcap2021/Data/Respiration/Runs" #location of files
```
...
```{r, warning=FALSE, message=FALSE}
Sample.Info<-read.csv("Mcap2021/Data/Respiration/Resp_Sample_Info.csv") #information on life stage, number of individuals, runs, etc.
...
starttimes<-read.csv("Mcap2021/Data/Respiration/starttimes.csv") #read in file of starting R and P phase times
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Resp.Data.N))){
    ...
    pdf(paste0("Mcap2021/Output/Respiration/RespirationPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output file names
    plot(model)
    dev.off()
...
write.csv(Resp.R, paste0("Mcap2021/Output/Respiration/Respiration_rates_Mcap2021.csv", sep="")) #save respiration rate data
```

```{r, warning=FALSE, message=FALSE}
Resp.Rates <- read.csv(file = "Mcap2021/Output/Respiration/Respiration_rates_Mcap2021.csv") #read file back in so slopes don't have to be generated every time
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Photo.Data.N))){
    ...
    pdf(paste0("Mcap2021/Output/Respiration/PhotosynthesisPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output files
    plot(model)
    dev.off()
...
write.csv(Photo.R, paste0("Mcap2021/Output/Respiration/Photosynthesis_rates_Mcap2021.csv", sep="")) #save P rates
```

```{r, warning=FALSE, message=FALSE}
Photo.Rates <- read.csv(file = "Mcap2021/Output/Respiration/Photosynthesis_rates_Mcap2021.csv") #import P rates
...
Remove wells with double peaks or instances where slope extraction does not represent data.  
...
No apparent outliers in blanks to account for.  
...
Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("Mcap2021/Output/Respiration/oxygen_P_R_calc.csv", sep="")) #save final file
```
