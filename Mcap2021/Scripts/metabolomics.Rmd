...
ggsave("Mcap2021/Figures/Metabolomics/VIP_Abundance_C12.jpg", C13_vip_abund_plot, width=11, height=6)
```
...
We will next look at enrichment data to determine whether metabolites are differentially labeled at high temperature on a per atom basis. Enrichment is calculated on a per atom basis - in other words, how enriched are the carbon atoms in the molecule on average? 
...
Plot pool size in descending order by temperature treatment. 

```{r}
#make summary table
enrichment_plotting<-plyr::ddply(enrichment, c("treatment", "Compound"), summarise,
...
enrichment_plotting<-enrichment_plotting%>%arrange(Compound)
```
...
We wil next look at the proportion of metabolites that have any carbon labeled (labeled) vs those with no label (not labeled), expressed as a proportion. In other words, what proportion of each metabolite in the pool has a label incorporated?
...
There are 23 metabolites with significant treatment:position interactions. 
...
First generate means for metabolites with significant interctive effects from above. 
```{r}
# create a vector of compound names
compound_list <- c(levels(as.factor(sign_position_metabolites$Compound)))
...
# Is metabolite turnover different between temperature treatments? 
