...
pool_ammonia_metabolites<-pool_size_plotting%>%
  filter(Compound %in% c("Glutamate", "Glutamine", "a-ketoglutarate", "N-acetylglutamate", "Carbamoyl phosphate", "Citrulline", "Aspartate", "Arginino-succinate", "Arginine", "Ornithine", "Glutamine-arginine"))%>%
...
ggsave("Mcap2021/Figures/Metabolomics/Controls/poolsize_isotope_nitrogen.jpg", plot=pool_ammonia_metabolites, width=5, height=5)
```
...
```{r}
control_list<-c("Methionine sulfoxide","Glutamate", "Glutamine", "a-ketoglutarate", "N-acetylglutamate", "Carbamoyl phosphate", "Citrulline", "Aspartate", "Arginino-succinate", "Arginine", "Ornithine", "Glutamine-arginine", "Glucose", "Fructose-6-phosphate", "Glucose-6-phosphate", "Pyruvate", "Dihydroxyacetone phosphate", "3-Phosphoglycerate", "Lactate", "Ribose phosphate", "Xylose-5-phosphate", "Citrate", "Isocitrate", "a-ketoglutarate", "Succinate", "Malate")
```
...
Perhaps this is showing slight stress (methionine sulfoxide?) and slight changes in TCA cycle? Look at this more with enrichment data.  
...
GS-GOGAT metabolites are much higher in light. Citrulline is the only one with more label in dark. 
...
enrichment%>%
  ...
  summarise(mean = mean(enrichment, na.rm=TRUE), se=sd(enrichment, na.rm=TRUE)/sqrt(length(enrichment)))
```
Ornithine is not significant, Citrulline is not significant, Carbamoyl phosphate has no label, Arginine is not significant. All others are P<0.001. 
