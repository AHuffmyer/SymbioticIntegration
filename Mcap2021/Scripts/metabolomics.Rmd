...
```{r}
#make summary table
data_plotting<-plyr::ddply(pool_size, c("isotope", "Compound", "treatment"), dplyr::summarise,
...
```{r}
#make summary table
pool_size_plotting<-plyr::ddply(pool_size, c("Compound", "isotope"), dplyr::summarise,
                N    = length(log.intensity.norm[!is.na(log.intensity.norm)]),
                mean = mean(log.intensity.norm, na.rm=TRUE),
                sd   = sd(log.intensity.norm, na.rm=TRUE),
                se   = sd / sqrt(N)
...
pool_size%>%
  filter(Compound %in% c("Glucose", "Fructose-6-phosphate", "Glucose-6-phosphate", "Pyruvate", "Dihydroxyacetone phosphate", "3-Phosphoglycerate", "Lactate", "Ribose phosphate", "Xylose-5-phosphate", "Citrate", "Isocitrate", "a-ketoglutarate", "Succinate", "Malate"))%>%
  filter(!isotope=="Dark-13C")%>%
  select(Compound, sample, isotope, treatment, tank, tube, log.intensity.norm)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS5a_data.csv")
```
...
pool_size%>%
  ...
  filter(!isotope=="Dark-13C")%>%
  select(Compound, sample, isotope, treatment, tank, tube, log.intensity.norm)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS5b_data.csv")
```
...
pool_size%>%
  dplyr::group_by(Compound, isotope)%>%
  dplyr::summarise(mean = mean(log.intensity.norm, na.rm=TRUE))%>%
  dplyr::arrange(Compound)%>%
  dplyr::filter(Compound %in% control_list)%>%
  dplyr::group_by(Compound)%>%
  pivot_wider(names_from=isotope, values_from=mean)%>%
  dplyr::mutate(perc_diff_12vs13=((`13C`-`12C`)/`12C`)*100)
```
...
pool_size%>%
  ...
  dplyr::summarise(mean = mean(log.intensity.norm, na.rm=TRUE), se=sd(log.intensity.norm, na.rm=TRUE)/sqrt(length(log.intensity.norm)))
...
pool_size%>%
  ...
  pivot_wider(names_from=Compound, values_from=log.intensity.norm)%>%
  filter(isotope=="12C")%>%
  write_csv("Mcap2021/Output/supp_data_for_figures_publication/FigS4a_data.csv")
```
...
pool_size%>%
  ...
  pivot_wider(names_from=Compound, values_from=log.intensity.norm)%>%
  filter(isotope=="13C")%>%
  write_csv("Mcap2021/Output/supp_data_for_figures_publication/FigS4b_data.csv")
```
...
pool_size%>%
  dplyr::select(Compound, isotope, treatment, tank, log.intensity.norm)%>%
  filter(isotope=="13C")%>%
  spread(Compound, log.intensity.norm)%>%
  write_csv("Mcap2021/Output/supp_data_for_figures_publication/Fig5a_data.csv")
...
# output data to supplement 
treatment_VIP_1 %>%
    arrange(VIP) %>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS8a_data.csv")
```
...
C13_vip_diff_plot<-C13_vip_metabolites%>%
  arrange(Compound)%>%
  group_by(Compound, treatment)%>%
  dplyr::summarise(mean_intensity=mean(log.intensity.norm))%>%
...
C13_vip_metabolites%>%
  arrange(Compound)%>%
  select(!intensity.norm)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/Fig5b_data.csv")
```
...
C13_vip_metabolites%>%
  arrange(Compound)%>%
  group_by(Compound, treatment)%>%
  dplyr::summarise(mean=mean(log.intensity.norm))%>%
  group_by(Compound)%>%
...
pool_size_plotting<-plyr::ddply(pool_size, c("treatment", "Compound", "isotope"), dplyr::summarise,
                N    = length(log.intensity.norm[!is.na(log.intensity.norm)]),
                mean = mean(log.intensity.norm, na.rm=TRUE),
                sd   = sd(log.intensity.norm, na.rm=TRUE),
                se   = sd / sqrt(N)
...
pool_size%>%
  ...
  filter(Compound=="Glucose")%>%
  select(!intensity.norm)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/Fig6a_data.csv")
```
...
enrichment%>%
  group_by(isotope)%>%
  dplyr::summarise(mean=mean(enrichment, na.rm=TRUE))
```
...
```{r}
#make summary table
enrichment_plotting<-plyr::ddply(enrichment, c("treatment", "Compound", "isotope"), dplyr::summarise,
...
# output data for supplement 
enrichment_pca_13C%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS9_data.csv")
...
```{r}
#make summary table
dark_plotting<-plyr::ddply(enrichment, c("Compound", "isotope"), dplyr::summarise,
                N    = length(enrichment[!is.na(enrichment)]),
                mean = mean(enrichment, na.rm=TRUE),
                sd   = sd(enrichment, na.rm=TRUE),
                se   = sd / sqrt(N)
...
enrichment %>%
  filter(Compound %in% c(c("Glucose", "Fructose-6-phosphate", "Glucose-6-phosphate", "Pyruvate", "Dihydroxyacetone phosphate", "3-Phosphoglycerate", "Lactate", "Ribose phosphate", "Xylose-5-phosphate", "Citrate", "Isocitrate", "a-ketoglutarate", "Succinate", "Malate")))%>%
  filter(!isotope=="12C")%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS6a_data.csv")
```
...
enrichment %>%
  filter(Compound %in% c("Glutamate", "Glutamine", "a-ketoglutarate", "N-acetyl-glutamate", "Citrulline", "Aspartate", "L-arginino-succinate", "Arginine", "Ornithine", "Arginine-Glutamine"))%>%
  filter(!isotope=="12C")%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS6b_data.csv")
```
...
enrichment%>%
  ...
  mutate(perc_diff_13vsdark=((`13C`-`Dark-13C`)/`13C`)*100)%>%
  ungroup()%>%
  dplyr::summarise(mean=mean(perc_diff_13vsdark, na.rm=TRUE), se=sd(perc_diff_13vsdark, na.rm=TRUE)/length(perc_diff_13vsdark))
...
all_dark<-enrichment%>%
  group_by(Compound, isotope)%>%
  dplyr::summarise(mean = mean(enrichment, na.rm=TRUE), se=sd(enrichment, na.rm=TRUE)/length(enrichment))
...
enrichment%>%
  ...
  filter(!isotope=="12C")%>%
  group_by(isotope)%>%
  dplyr::summarise(mean = mean(enrichment, na.rm=TRUE), se=sd(enrichment, na.rm=TRUE)/sqrt(length(enrichment)))
...
enrichment%>%
  dplyr::select(Compound, isotope, treatment, tank, enrichment)%>%
  filter(isotope=="13C")%>%
  spread(Compound, enrichment)%>%
  write_csv("Mcap2021/Output/supp_data_for_figures_publication/Fig5c_data.csv")
```
...
enrich_VIP_1 %>%
  arrange(VIP) %>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS8b_data.csv")
```
...
enrich_vip_diff_plot<-enrich_vip_metabolites%>%
  arrange(Compound)%>%
  group_by(Compound, treatment)%>%
  dplyr::summarise(mean=mean(enrichment))%>%
  group_by(Compound)%>%
  reframe(fold_change=(mean-first(mean))/first(mean))%>% #calculate the difference of high from ambient
  filter(fold_change !=0)%>% #remove zero values (comparing ambient to ambient)
...
enrich_vip_diff_sum<-enrich_vip_metabolites%>%
  ...
  dplyr::summarise(mean=mean(enrichment))%>%
  group_by(Compound)%>%
  dplyr::summarise(fold_change=(mean-first(mean))/first(mean))%>% #calculate the difference of high from ambient
  filter(fold_change !=0)

write.csv(enrich_vip_diff_sum, file="Mcap2021/Output/Metabolomics/VIP_Enrichment_RelDiff.csv")

#output data for supplement
enrich_vip_metabolites%>%
  arrange(Compound)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/Fig5d_data.csv")
```
...
```{r}
  ...
  geom_text(label="VIP=0.38", x=1.5, y=0.65, size=8, colour="darkgray")+
...
enrichment%>%
  filter(isotope=="13C")%>%
  filter(Compound=="Glucose")%>%
  write_csv("Mcap2021/Output/supp_data_for_figures_publication/Fig6b_data.csv")
```
...
position_plotting<-plyr::ddply(fractions, c("treatment","Position", "Compound", "isotope"), dplyr::summarise,
...
fractions%>%
  ...
  filter(!Position==0)%>%
  filter(Compound=="Glucose")%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/Fig6c_data.csv")
```
...
Output supplementary data for other compounds of interest. 
```{r}
#output data for supplement 
fractions%>%
  filter(isotope=="13C")%>%
  filter(!Position==0)%>%
  filter(Compound %in% c("Glucose-6-phosphate", "Fructose-6-phosphate", "Pyruvate", "Aconitate"))%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/FigS10_data.csv")
```
...
master_treatment%>%
  filter(!Compound=="Montiporic Acid C")%>%
  select(Compound, change_pool, change_enrichment)%>%
  write_csv(file="Mcap2021/Output/supp_data_for_figures_publication/Fig3b_data.csv")
```
