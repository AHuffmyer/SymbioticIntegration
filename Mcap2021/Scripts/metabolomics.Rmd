...
metab_compounds_isotopes_acids<-data_plotting%>%
  ...
  scale_fill_manual(values=c("gray", "lightgreen", "darkblue"))+
  theme_classic()+
  xlab("Metabolites")+
...
ggsave("Mcap2021/Figures/Metabolomics/pool_size_compounds.jpg", plot=poolsize_fig, width=26, height=10)
```
...
ggsave("Mcap2021/Figures/Metabolomics/pool_size_compounds_temperature.jpg", plot=poolsize_temp_fig, width=26, height=10)
```
...
pcaPlot1<-ggplot2::autoplot(scaled.pca.ambient, data=pool_size_pca_ambient, frame.colour="isotope", loadings=FALSE, colour="isotope", loadings.label.colour="black", loadings.colour="black", loadings.label=FALSE, frame=FALSE, loadings.label.size=5, loadings.label.vjust=-1, size=5) + 
  ...
  theme_classic()+
  scale_colour_manual(values=c("gray", "lightgreen", "darkblue"))+
  scale_fill_manual(values=c("gray", "lightgreen", "darkblue"))+
...
ggsave("Mcap2021/Figures/Metabolomics/pca_isotope.jpg", pca_isotope_grid, width=5, height=8)
```
...
There is significant variation in the PCA by isotope but response to temperature is not affected by isotope - so we can make comparisons between C13 by treatment. 
...
```{r}
#make summary table
pool_size_plotting<-plyr::ddply(pool_size, c("Compound", "isotope", "treatment"), summarise,
                N    = length(log.intensity.norm[!is.na(log.intensity.norm)]),
                mean = mean(log.intensity.norm, na.rm=TRUE),
                sd   = sd(log.intensity.norm, na.rm=TRUE),
                se   = sd / sqrt(N)
...
pool_size_plotting<-pool_size_plotting%>%arrange(Compound)#%>%filter(!isotope=="12C")
```
...
pool_photo_metabolites<-pool_size_plotting%>%
  ...
  theme_classic()+
  scale_fill_manual(values=c("gray", "lightgreen", "darkblue"))+
  scale_x_discrete(limits=c("Glucose", "Glucose-6-phosphate", "Fructose-6-phosphate", "Pyruvate"))+
  xlab("")+
  ylab("Pool Size")+
  #ylim(0,0.25)+
  theme(
...
ggsave("Mcap2021/Figures/Metabolomics/poolsize_isotope_photosynthesis.jpg", plot=pool_photo_metabolites, width=5, height=8)
```

```{r}
pool_ammonia_metabolites<-pool_size_plotting%>%
  filter(Compound %in% c("Glutamate", "Glutamine", "a-ketoglutarate"))%>%
...
ggsave("Mcap2021/Figures/Metabolomics/poolsize_isotope_ammonia.jpg", plot=pool_ammonia_metabolites, width=5, height=8)
```

```{r}
pool_tca_metabolites<-pool_size_plotting%>%
  filter(Compound %in% c("Citrate", "a-ketoglutarate", "Aconitrate", "Isocitrate", "Succinate", "Malate"))%>%
  ...
  theme(
    ...
    axis.text.x = element_text(size=12, color="black", angle=45, vjust = 1, hjust=1), 
    axis.title = element_text(face="bold")
    );pool_tca_metabolites

ggsave("Mcap2021/Figures/Metabolomics/poolsize_isotope_tca.jpg", plot=pool_tca_metabolites, width=5, height=8)
```
...
ggsave("Mcap2021/Figures/Metabolomics/poolsize_isotope_methioninesulfoxide.jpg", plot=pool_host_metabolites, width=3, height=5)
```

Calculate average difference in labeling between isotope for these key metabolites. "There was X% decrease in pool size in C12 vs C13 samples."
```{r}
...
pool_size%>%
  ...
  arrange(Compound)%>%
  filter(Compound %in% c("Citrate", "a-ketoglutarate", "Aconitrate", "Isocitrate", "Succinate", "Malate", "Glutamate", "Glutamine", "a-ketoglutarate", "Glucose", "Glucose-6-phosphate", "Fructose-6-phosphate", "Pyruvate"))%>%
  group_by(Compound)%>%
  pivot_wider(names_from=isotope, values_from=mean)%>%
  mutate(perc_diff_12vs13=((`13C`-`12C`)/`13C`)*100)%>%
...
pool_size%>%
  ...
  mutate(perc_change_13vsdark=((`13C`-`Dark-13C`)/`13C`)*100)%>%
  ungroup()%>%
  summarise(mean_12vs13=mean(perc_change_12vs13), 
            mean_13vsdark=mean(perc_change_13vsdark))
```

Calculate average pool size for metabolites of interest and then all metabolites by isotope. 
```{r}
pool_size%>%
  filter(Compound %in% c("Citrate", "a-ketoglutarate", "Aconitrate", "Isocitrate", "Succinate", "Malate", "Glutamate", "Glutamine", "a-ketoglutarate", "Glucose", "Glucose-6-phosphate", "Fructose-6-phosphate", "Pyruvate", "Methionine sulfoxide"))%>%
  group_by(isotope)%>%
...
#test difference in label by isotope for all compounds 
test2<-pool_size%>%
...
emm<-emmeans(test2, ~isotope | Compound)
pairs(emm)
```

Isocitrate (P<0.001), citrate (P=0.010), succinate (P=0.034), and methionine sulfoxide (P<0.001)  were higher in C13 than C12. 
...
pool_size%>%
  ...
  summarise(mean = mean(log.intensity.norm, na.rm=TRUE))%>%
  arrange(Compound)%>%
  filter(Compound %in% c("Citrate", "Isocitrate", "Glutamate"))%>%
  group_by(Compound)%>%
  pivot_wider(names_from=isotope, values_from=mean)%>%
  mutate(perc_change_darkvs13=((`13C`-`Dark-13C`)/`13C`)*100)
```
...
treatment_cols<-c("gray", "lightgreen") 
...
C13_vip_diff_plot<-C13_vip_metabolites%>%
  ...
  scale_color_manual(values=c("blue", "red"), name="Treatment")+
  ylab("Rel. difference in pool size")+
  xlab("VIP Metabolite")+
...
```{r}
  glucose.pool.plot <- pool_size_plotting%>%
    ...
    geom_text(label="*", x=1.5, y=3.3, size=8)+ 
...
C12_vip_diff_plot<-C12_vip_metabolites%>%
  ...
  geom_bar(stat="identity",position=position_jitterdodge(0.2), fill="white")+
  scale_color_manual(values=c("blue", "red"), name="Treatment")+
  ylab("Rel. difference in log(normalized pool size)")+
  xlab("VIP Metabolite")+
...
```{r}
#make summary table
dark_plotting<-plyr::ddply(enrichment, c("Compound", "isotope", "treatment"), summarise,
                N    = length(enrichment[!is.na(enrichment)]),
                mean = mean(enrichment, na.rm=TRUE),
                sd   = sd(enrichment, na.rm=TRUE),
                se   = sd / sqrt(N)
...
ggsave("Mcap2021/Figures/Metabolomics/enrichment_darkvslight_all.jpg", plot=dark_all_metabolites, width=26, height=12)
```

Plot by isotope with comparison by temprature. 

```{r}
dark_all_metabolites_temp<-dark_plotting%>%
  ...
  theme(
    ...
    axis.text.x = element_text(size=12, color="black", angle=90, vjust = 0.5, hjust=1), 
    axis.title = element_text(face="bold")
    );dark_all_metabolites_temp

ggsave("Mcap2021/Figures/Metabolomics/enrichment_darkvslight_all_temp.jpg", plot=dark_all_metabolites_temp, width=26, height=12)
```
...
ggsave("Mcap2021/Figures/Metabolomics/enrichment_darkvslight_photosynthesis.jpg", plot=dark_photo_metabolites, width=5, height=8)
```
...
ggsave("Mcap2021/Figures/Metabolomics/enrichment_darkvslight_ammonia.jpg", plot=dark_ammonia_metabolites, width=5, height=8)
```

```{r}
dark_tca_metabolites<-dark_plotting%>%
  filter(Compound %in% c("Citrate", "a-ketoglutarate", "Aconitrate", "Isocitrate", "Succinate", "Malate"))%>%
  ...
  theme(
    ...
    axis.text.x = element_text(size=12, color="black", angle=45, vjust = 1, hjust=1), 
    axis.title = element_text(face="bold")
    );dark_tca_metabolites

ggsave("Mcap2021/Figures/Metabolomics/enrichment_darkvslight_tca.jpg", plot=dark_tca_metabolites, width=5, height=8)
```

```{r}
dark_urea_metabolites<-dark_plotting%>%
  filter(Compound %in% c("N-acetyl-glutamate", "Citrulline", "Aspartate", "L-arginino-succinate", "Arginine", "Ornithine", "Carbamoyl phosphate"))%>%
  ...
  theme(
    ...
    axis.text.x = element_text(size=12, color="black", angle=45, vjust = 1, hjust=1), 
    axis.title = element_text(face="bold")
    );dark_urea_metabolites

ggsave("Mcap2021/Figures/Metabolomics/enrichment_darkvslight_urea.jpg", plot=dark_urea_metabolites, width=8, height=8)
```
...
There are some metabolites that are highly labeled in the dark - including methionine sulfoxide, citrulline, uridine, tryptophan, UMP, L-argino-succinate, tyrosine, and inosine. This suggests that the labeled carbon in these compounds did not originate through photosynthesis and are instead host derived. 
...
ggsave("Mcap2021/Figures/Metabolomics/enrichment_darkvslight_methioninesulfoxide.jpg", plot=dark_ms_metabolites, width=3, height=5)
```

Calculate average difference in labeling between isotope for these key metabolites. "There was X% decrease in labeling C12 vs C13 samples."
```{r}
#show for each compound 
enrichment%>%
  group_by(Compound, isotope)%>%
...
#run a test by temperature including citrulline for urea cycle
test<-enrichment%>%
  filter(Compound %in% c("Citrate", "a-ketoglutarate", "Aconitrate", "Isocitrate", "Succinate", "Malate", "Glutamate", "Glutamine", "a-ketoglutarate", "Glucose", "Glucose-6-phosphate", "Fructose-6-phosphate", "Pyruvate", "Citrulline"))%>%
  aov(enrichment~isotope*Compound*treatment, data=.)

summary(test)

emm<-emmeans(test, ~isotope * treatment | Compound)
pairs(emm)
...
We cannot run a PCA because the C12 samples have no variance (all 0's) for most metabolites. 
...
enrich_vip_diff_plot<-enrich_vip_metabolites%>%
  ...
  geom_bar(stat="identity",position=position_jitterdodge(0.2), fill="white")+
  scale_color_manual(values=c("blue", "red"), name="Treatment")+
  ylab("Rel. difference in enrichment")+
  xlab("VIP Metabolite")+
...
label_vip_diff_plot<-label_vip_metabolites%>%
  ...
  geom_bar(stat="identity",position=position_jitterdodge(0.2), fill="white")+
  scale_color_manual(values=c("blue", "red"), name="Treatment")+
  ylab("Rel. difference in labeling")+
  xlab("VIP Metabolite")+
