...
```{r, warning=FALSE, message=FALSE}
path.p<-"Pacu2021/Data/Respiration/Stages/Runs" #location of files
```
...
```{r, warning=FALSE, message=FALSE}
Sample.Info<-read.csv("Pacu2021/Data/Respiration/Stages/Resp_Sample_Info_Stages.csv") #information on life stage, number of individuals, runs, etc.
...
starttimes<-read.csv("Pacu2021/Data/Respiration/Stages/starttimes_stages.csv") #read in file of starting R and P phase times
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Resp.Data.N))){
    ...
    pdf(paste0("Pacu2021/Output/Respiration/Stages/RespirationPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output file names
    plot(model)
    dev.off()
...
Removed column B6 due to missing sensor in 710 plates. This column had to be removed from data sheets due to 'no sensor' reading that stops the code. B6 removed from all plates to allow loop to run.  
...
write.csv(Resp.R, paste0("Pacu2021/Output/Respiration/Stages/Respiration_rates_Pacu2021.csv", sep="")) #save respiration rate data
```

```{r, warning=FALSE, message=FALSE}
Resp.Rates <- read.csv(file = "Pacu2021/Output/Respiration/Stages/Respiration_rates_Pacu2021.csv") #read file back in so slopes don't have to be generated every time
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  Photo.Data.N <- Photo.Data[,3:25] #select desired columns
  ...
  for(j in 1:(ncol(Photo.Data.N))){
    ...
    pdf(paste0("Pacu2021/Output/Respiration/Stages/PhotosynthesisPlots/",date[i], "_",run[i],"_",rename[j],"_regression_trunc.pdf")) #generate output files
    plot(model)
    dev.off()
...
write.csv(Photo.R, paste0("Pacu2021/Output/Respiration/Stages/Photosynthesis_rates_Pacu2021.csv", sep="")) #save P rates
```

```{r, warning=FALSE, message=FALSE}
Photo.Rates <- read.csv(file = "Pacu2021/Output/Respiration/Stages/Photosynthesis_rates_Pacu2021.csv") #import P rates
...
Remove wells that slope extraction does not match data (multiple peaks, wells with issues).  

```{r}
oxygen$code<-paste(oxygen$Run, oxygen$Chamber.ID)
remove<-c("1 C2", "2 D5", "3 B2", "3 B4", "3 D4", "4 C6", "4 D5")
oxygen<-oxygen%>%
...
Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("Pacu2021/Output/Respiration/Stages/oxygen_P_R_calc.csv", sep="")) #save final file
```
