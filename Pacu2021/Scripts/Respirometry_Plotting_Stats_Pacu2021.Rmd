...
r_plot<-PRdata %>%
    ...
    geom_point(aes(fill=Temperature, group=Temperature), pch = 21, size=6, alpha=0.5, position = position_jitterdodge(0.3)) + 
...
Rmodel1<-lmer(R.nmol.org.min~Temperature + (1|Run/Date), data=PRdata) #run as random
...
```{r, results=TRUE, warning=FALSE, message=FALSE}
Pmodel1<-lmer(P.nmol.org.min~Temperature + (1|Run/Date), data=PRdata) #run nested within day
...
```{r, results=TRUE, warning=FALSE, message=FALSE}
hist(PRdata$GP.umol.org.min)
GPmodel1<-lmer(GP.nmol.org.min~Temperature + (1|Run/Date), data=PRdata) #run nested within day
...
```{r, results=TRUE, warning=FALSE, message=FALSE}
PRmodel1<-lmer(ratio~Temperature + (1|Run/Date), data=PRdata) #run as random
